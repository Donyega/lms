      ...
      <div class="row breadcrumbs-top">
        <div class="col-12">
          ...
          <div class="breadcrumb-wrapper">
            <ol class="breadcrumb">
              <li class="breadcrumb-item"><a href="{{route('laporan.presensi')}}">Presensi</a></li>
    ...
    <div class="row">
              ...
              <div class="col-lg-6 col-sm-12">
                <div class="table-responsive-lg">
                  <table class="table table-striped">
                    ...
                    <tr>
                      <td>Jenis Kelas</td>
                      <td><b>{{$data->kelas->jenis->nama}}</b></td>
                    </tr>
                    <tr>
      ...
      <div class="col-12">
        <div class="text-center text-lg-left mb-1">
          ...
          <a href="{{route('laporan.presensi.detailpresensi', [$data->id,0])}}" class="btn btn-outline-primary" style="white-space:nowrap" target="_blank"><i data-feather="check-square"></i> Detail Presensi</a>
          <div class="btn-group">
            ...
            <div class="dropdown-menu">
              ...
              <form class="dropdown-item" action="{{route('laporan.presensi.rekap')}}" target="_blank" method="post"><input type="hidden" name="idjadwal" value="{{$data->id}}">
...
<div class="modal fade" id="modal-absen" tabindex="false" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <form id="jquery-val-form" class="forms-sample formsws2" action="{{route('laporan.presensi.store')}}" method="post">
        <div class="modal-body">
          <input type="hidden" name="idjadwal" value="{{$data->id}}">
          <input type="hidden" name="isagama" value="0">
          <div class="row">
            ...
            <div class="form-group col-lg-6">
              ...
              <div class="col-sm-12">
                <input type="text" class="form-control" value="{{$data->kelas->nama}} {{$data->kelas->jenis->nama}}" readonly>
              </div>
            </div>
            <div class="form-group col-lg-6">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-12">
              <?php
                $jmlguru = $data->jadwalguru->count();
              ?>
              <select name="idguru" class="form-control select2" data-placeholder="Pilih" required>
                <option></option>
                @if (auth::user()->role == 2 && auth::user()->can('pejabat') == false)
                  @foreach ($data->jadwalguru->where('idguru',auth::user()->link) as $jg)
                    <option selected value="{{$jg->idguru}}">{{$jg->guru->nama}}</option>
                  @endforeach
                @else
                  @foreach ($data->jadwalguru as $jg)
                    <option {{$jmlguru == 1 ? 'selected' : ''}} value="{{$jg->idguru}}">{{$jg->guru->nama}}</option>
                  @endforeach
                @endif
              </select>
            </div>
