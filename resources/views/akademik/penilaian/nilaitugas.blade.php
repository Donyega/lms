...
<title>SLUA | Laporan Nilai Tugas - {{$mapel->nama}} ({{$namakelas}})</title>
<style media="screen">
  table {
...
<main>
  ...
  <center>
    <h3 style="margin-bottom: 0px; text-decoration:underline">REKAPITULASI NILAI TUGAS</h3>
        ...
        <tr>
          ...
          <td style="white-space: nowrap">
            @foreach ($penugasan->where('idjenis', 1) as $p)
              @php
                $nilai = App\Models\LmsSiswaTugas::where('idpenugasan', $p->id)->where('nis', $s->nis)->get();
              @endphp
              @foreach ($nilai as $n)
                {{$n->nilai}}
              @endforeach
